...
- render separators inside grouped cards using the page background colour;
...
their neutral outer outline is removed. Grouped rows retain a one-pixel
separator using `var(--bg)`, which reads as a narrow gap cut from the page.
...
theme tiles, the core-version list, and the split-tunnel application picker.
